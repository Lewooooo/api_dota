...
class _HeroListScreenState extends State<HeroListScreen> with SingleTickerProviderStateMixin {
  ...
  void _toggleFavorite(int heroId) async {
    setState(() {
      if (favoriteIds.contains(heroId)) {
        favoriteIds.remove(heroId);
        FavoritesService.removeFavorite(heroId);
      } else {
        favoriteIds.add(heroId);
        FavoritesService.addFavorite(heroId);
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<DotaHero>>(
        ...
        builder: (context, snapshot) {
          ...
          final favoriteHeroes = heroes.where((hero) => favoriteIds.contains(hero.id)).toList();

          return TabBarView(
            controller: _tabController,
            children: [
              _buildHeroGrid(heroes),
              _buildHeroGrid(favoriteHeroes),
            ],
          );
        },
      ),
  ...
  Widget _buildHeroGrid(List<DotaHero> heroes) {
    ...
    List<DotaHero> filtered = heroes.where((h) {
      ...
      final matchRole = selectedRoles.isEmpty ||
                    ...
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (_) => HeroDetailScreen(
                          ...
                          onFavoriteToggle: () => _toggleFavorite(hero.id),
                        ),
                      ),
                    );
                  },
                    ...
                    children: [
                      ...
                      Positioned(
                        ...
                        child: IconButton(
                          ...
                          onPressed: () => _toggleFavorite(hero.id),
                        ),
                      ),
                    ],
                  ),
